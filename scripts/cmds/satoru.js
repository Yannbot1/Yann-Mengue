€cmd install satoru.js const axios = require('axios');
...
async function getAIResponse(input, userId, messageID) {
    ...
    let response = " ✰.. 𝘚𝘢𝘭𝘶𝘵 👋 𝘦𝘯 𝘲𝘶𝘰𝘪 𝘱𝘶𝘪𝘴-𝘫𝘦 𝘷𝘰𝘶𝘴 𝘢𝘪𝘥𝘦𝘻 ?? .. écrit €gojo pour rejoindre mon groupe ✰";
    let currentIndex = 0;
...
category:'david',
    ...
    onStart: async function ({ api, event, args }) {
        ...
        if (!input) {
            api.sendMessage(`𝐸𝑄𝑈𝐴𝑇𝐼𝑂𝑁 𝐼𝑀𝐴𝐺𝐼𝑁𝐴𝐼𝑅𝐸 𝑉𝐼𝑂𝐿𝐸𝑇 🌌 !`, event.threadID, event.messageID);
            return;
        }

        const { response, messageID } = await getAIResponse(input, event.senderID, event.messageID);
        api.sendMessage(`𝙎𝘼𝙏𝙊𝙍𝙐 𝙂𝙊𝙅𝙊 𝙚𝙨𝙩 𝙩𝙧𝙤𝙥 𝙘𝙤𝙤𝙡 !`, event.threadID, messageID);
    },
    onChat: async function ({ event, message }) {
        ...
        if (messageContent.startsWith("1")) {
            ...
            const formattedResponse = ` ༒𝑆𝐴𝑇𝑂𝑅𝑈✫ 𝐺𝑂𝐽𝑂✯ 𝐵𝑂𝑇༒:\n━━━━━━━━━━━━━━━━\n${response} `;
